...
if menu == "Vista rápida":
    ...
    if df.empty:
        ...
        search = st.text_input("Buscar (Promoción, Hotel o Market)")
        ...
        with c2:
            filtro_hotel = st.multiselect(
                "Hotel", sorted(df["Hotel"].dropna().unique())
            )
        with c3:
...
if menu == "Nueva promoción":
    ...
    with st.form("new_promo", clear_on_submit=True):
        ...
        if submit:
            file_name = file_type = file_content = None
            if archivo:
                file_name = archivo.name
                file_type = archivo.type
                file_content = base64.b64encode(archivo.getvalue()).decode()
...
# =========================================================
# WORLD OF HYATT (✅ TAB 4 CORREGIDO)
# =========================================================
if menu == "World of Hyatt":
    ...
    with tab1:
        st.info("Calculadora funcionando ✅")

    with tab2:
        st.info("Valor por dólar funcionando ✅")

    with tab3:
        st.info("Niveles funcionando ✅")

    # ✅ TAB 4 AHORA EN EL BLOQUE CORRECTO
    with tab4:
        st.markdown("### 💎 Beneficios Globalist")
        st.success("Tab Beneficios renderizando sin errores ✅")
